...
class C0 extends T0, T1 {
	...
	function fm2(globalState: GlobalState): bool {
		(0x388 % globalState.f0) > globalState.f0
	}
	function fm3(p0: bool, p1: bool, globalState: GlobalState): int {
		-((if ((if (globalState.f9 <==> (p0 ==> true)) then 0x92 else globalState.f7) >= 17) then -globalState.f5 else globalState.f4) - (-globalState.f7 / globalState.f2))
	}
	function fm4(p0: int, p1: bool, p2: set<bool>, p3: int, p4: int, p5: bool, p6: int, p7: int, p8: map<bool, bool>, p9: int, p10: int, p11: bool, p12: bool, p13: int, globalState: GlobalState): map<int, int> {
		map[globalState.f4 := p13] + map[globalState.f5 := p9 / |[-0x272]|]
	}
	method m0(p0: bool, p1: bool, p2: bool, p3: int, p4: bool, globalState: GlobalState) returns (r0: bool, r1: int, r2: int, r3: int, r4: int, r5: bool, r6: int, r7: seq<int>, r8: int) {
		var v0: multiset<int> := multiset{(0x3c7 - globalState.f2) % 0x3a1, -f16};
		f13 := if (f16 in v0) then v0[f16] else globalState.f0;
		r0 := 0x81 >= ((globalState.f7 / 0x19e) - globalState.f4);
		r1 := p3 / globalState.f5;
		r2 := 0x295;
		r3 := globalState.f2 + f16;
		r4 := -(|{f12} + {false}| - f16);
		r5 := true;
		r6 := globalState.f5 / -0xf4;
		r7 := [globalState.f7 / f13, 0xf2, globalState.f4, globalState.f0] + ([-0x1a1, 167, f16] + [f15]);
		r8 := f15;
	}
...
method Main() {
	...
	while (v1.fm0(v1.fm2(globalState), if (false) then 432 * v1.f17 else if (((if (globalState.f0 in v3) then v3[globalState.f0] else globalState.f0) / globalState.f7) in v3) then v3[(if (globalState.f0 in v3) then v3[globalState.f0] else globalState.f0) / globalState.f7] else globalState.f4 - globalState.f0, |v4| - 0x33b, false <==> true, globalState)) {
		...
		v5[565] := globalState.f0;
		globalState.f1[68] := !globalState.f8;
		...
		globalState.f1[892] := 0x254 > globalState.f4;
	}
	...
	var v44 := new C0(0x28a * (if (if (globalState.f6 in v26) then v26[globalState.f6] else globalState.f9) then globalState.f2 % v1.f17 else -v1.f17), globalState.f8, globalState.f6, true, if (true) then 320 else (v30.f17 / globalState.f3) / -0x22e, globalState.f8, globalState.f5, if (globalState.f6) then (globalState.f5 + |v43|) + globalState.f4 else 982 % globalState.f4);
}